...
const BACKEND_URL='https://script.google.com/macros/s/AKfycbz5voiADC-aJtPFmdL6mVTc4hDAdG8ZzGqrESxwauSHr4-Tqzl6RUT_JuU-5t41nSBi2Q/exec';const money=n=>n.toLocaleString('ru-RU')+' UZS';
...
async function submitOrder(){
  ...
  if(!Array.isArray(cart)||cart.length===0){
    alert('Корзина пуста. Добавьте товар перед оформлением заказа.');
    return;
  }

  const items=cart.map(x=>{
    const p=PRODUCTS.find(p=>p.id===x.id);
    return p?{name:p.name,qty:Number(x.qty)||0,price:Number(p.price)||0}:null;
  }).filter(x=>x&&x.qty>0);

  if(items.length===0){
    alert('Не удалось определить товары в заказе.');
    return;
  }
  ...
  try{
    ...
    cart=[];
    save();
    document.getElementById('successText').textContent=`Ваш заказ ${order.orderId} принят. Мы свяжемся с вами для подтверждения.`;
    showScreen('success');
    tg?.HapticFeedback?.notificationOccurred('success');
  }catch(e){
