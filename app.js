...
let cart=JSON.parse(localStorage.getItem('layerone_cart')||'[]'),currentFilter='all',currentProduct=null,geo=null,orderSubmitting=false;const money=n=>n.toLocaleString('ru-RU')+' UZS';
...
async function submitOrder(){
  ...
  const buttons=[...document.querySelectorAll('button')].filter(b=>{
    ...
    return onclick.includes('submitOrder') || txt.includes('оформить заказ');
  });
  ...
  try{
    const items=cart.map(x=>{
      ...
      return{name:p.name,qty:x.qty,price:p.price};
    });

    const sum=items.reduce((s,x)=>s+x.price*x.qty,0);

    const order={
      orderId:'L1-'+Date.now().toString().slice(-6),
      ...
      items,
      total:sum,
      delivery:'Оплачивается покупателем при получении',
      createdAt:new Date().toISOString()
    };

    const url='https://script.google.com/macros/s/AKfycbwiPZNpI42UqCKpfZzqoRJsUsSrUj3EKehIJSRI1-icjV1DxE2eeoVp6E3bBJfKnTEbpg/exec';
    ...
    document.getElementById('successText').textContent=
      `Ваш заказ ${order.orderId} принят. Мы свяжемся с вами для подтверждения.`;
    ...
    buttons.forEach(b=>{
      b.disabled=false;
      b.textContent=b.dataset.oldText||'Оформить заказ';
    });
  }
}
